...
const EditShareHolderForm = ({ shareholder, onUpdateShareHolder, show, onHide }) => {
    ...
    const downloadScannedCertificate = async () => {
        try {
            const response = await http.get(`/shareholders/${Number(shareholder.id)}/download-file`, {
                responseType: 'blob',
            });
            const fileUrl = URL.createObjectURL(new Blob([response.data]));
            const link = document.createElement('a');
            link.href = fileUrl;
            link.setAttribute('download', 'scanned_certificate.pdf'); // Set the file name for the download
